...
static inline void bictcp_hystart_reset(struct sock *sk)
{
	...
	(ca->appendix)->round_start = (ca->appendix)->last_ack = bictcp_clock();
	(ca->appendix)->end_seq = tp->snd_nxt;
	(ca->appendix)->curr_rtt = 0;
	ca->sample_cnt = 0;
}
			      ...
			      u32 in_flight)
{
	...
	if (tp->snd_cwnd <= tp->snd_ssthresh) {
		if (hystart && after(ack, (ca->appendix)->end_seq))
			bictcp_hystart_reset(sk);
...
static void hystart_update(struct sock *sk, u32 delay)
{
	...
	if (!(ca->found & hystart_detect)) {
		...
		if ((s32)(now - (ca->appendix)->last_ack) <= hystart_ack_delta) {
			(ca->appendix)->last_ack = now;
			if ((s32)(now - (ca->appendix)->round_start) > ca->delay_min >> 4)
				ca->found |= HYSTART_ACK_TRAIN;
		}

		/* obtain the minimum delay of more than sampling packets */
		if (ca->sample_cnt < HYSTART_MIN_SAMPLES) {
			if ((ca->appendix)->curr_rtt == 0 || (ca->appendix)->curr_rtt > delay)
				(ca->appendix)->curr_rtt = delay;

			ca->sample_cnt++;
		} else {
			if ((ca->appendix)->curr_rtt > ca->delay_min +
...
MODULE_AUTHOR("Sangtae Ha, Stephen Hemminger, Joseph Beshay");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("JOSPEPH'S MODIFIED CUBIC TCP");
MODULE_VERSION("2.3");
